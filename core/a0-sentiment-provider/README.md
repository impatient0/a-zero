...
*   **Migration to Structured Concurrency:** The current API relies on `CompletableFuture` for asynchronous operations. With the project's upgrade to Java 25, this should eventually be refactored to a synchronous, blocking API that leverages Virtual Threads and Structured Concurrency (JEP 480). This will simplify the interface, improve stack trace readability, and align with modern Java best practices.