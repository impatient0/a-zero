...
The engine supports ingesting pre-processed sentiment data to allow strategies to make decisions based on market sentiment (e.g., news analysis).

-   **Data Ingestion:** The engine accepts a map of `SentimentSignal` lists.
-   **Time Alignment:** Sentiment data is indexed by time. The engine automatically "forward-fills" the sentiment state. If a signal arrives at `t=100`, it remains the active sentiment for that symbol until a new signal arrives or the simulation ends.
-   **Strategy Access:** Strategies can access the current sentiment via `context.getCurrentSentiment(symbol)`.
...
// 2b. Load sentiment data (Optional)
// This data is typically generated by the a0-sentiment-preprocessor-cli tool
Map<String, List<SentimentSignal>> sentimentData = CsvSentimentLoader.load(Path.of("BTC-sentiment.csv"));
